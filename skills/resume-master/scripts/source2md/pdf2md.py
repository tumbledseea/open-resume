...
def get_heading_level(size: float, size_map: dict, text: str = "",
    ...
    if text and text[-1] in sentence_endings:
        ...
        if not re.match(r'^(?:\d+|第[一二三四五六七八九十百千万0-9]+)[\.\、:：章节条]?', text):
            return 0
...
def remove_page_footer(text: str) -> str:
    """
    Remove page number patterns from footers, e.g. 'November 2025 8' or '2025骞?1鏈?8'.
    """
    ...
    # Chinese format: 2025骞?1鏈?8
    pattern_cn = r'\s*\d{4}骞碶d{1,2}鏈圽s+\d{1,3}\s*$'
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples:
  python pdf2md.py book.pdf                    # Convert a single file
  python pdf2md.py book.pdf -o output.md      # Specify output file
  python pdf2md.py ./pdfs                      # Convert all PDFs in directory
  python pdf2md.py ./pdfs -o ./markdown       # Specify output directory
