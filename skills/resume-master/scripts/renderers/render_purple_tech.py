#!/usr/bin/env python
"""Render template8 LaTeX from structured resume modules."""
...
TEMPLATE_ROOT = SKILL_ROOT / "examples" / "purple_tech"
...
def render_to_output(modules_path: Path, output_path: Path) -> tuple[Path, Path]:
    ...
    cls_output = output_path.parent / "purple_tech.cls"
    shutil.copy2(CLASS_PATH, cls_output)
    return output_path, cls_output


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render template8 LaTeX from resume modules")
